...
public:
	...
	weight_t GetWeight(vertex_t p1, vertex_t p2)
	{
		std::list<Edge> * adj = &adjacency_map[p1];
	...
	vertex_t findset(vertex_t x, std::vector<vertex_t> & parent)
	{
		if(x != parent[x]) parent[x] = findset(parent[x], parent);
		return parent[x];
	}

	Graph<vertex_t, weight_t> kruskal()
	{
		Graph<vertex_t, weight_t> graph;

		int N = vertices.size();

		typedef std::pair< weight_t, std::pair< vertex_t, vertex_t > > KruskalEdge;
		std::vector< KruskalEdge > GRAPH, MST;
		std::vector< vertex_t > parent(N+1, 0);

		// Add weighted edges
		foreach(Edge e, GetEdges())
		{
			GRAPH.push_back( std::make_pair(e.weight, std::make_pair(e.index, e.target)) );
		}
		...
		foreach(KruskalEdge edg, MST)
		{
			vertex_t id1 = edg.second.first;
			vertex_t id2 = edg.second.second;
			weight_t weight = edg.first;
			graph.AddEdge( graph.AddVertex(id1), graph.AddVertex(id2), weight, id1 );
		}
