...
struct FrontEdge {     
	int v0, v1, v2;  	// v0, v1 represent the FrontEdge, v2 the other vertex in the face this FrontEdge belongs to    
...
struct SimpleFace{ int v[3]; int & operator[](const int& vi) { return v[vi]; } };
...
public:     
	...
	SurfaceMeshModel * mesh;	// Initially mesh only have vertices
	KDTree * grid;				// lookup structure for points

	std::vector<SimpleFace> fV;

	Surface_mesh::Vertex_property<Vec3d> points, real_points;
	Surface_mesh::Vertex_property<bool> usedBit;
	Surface_mesh::Vertex_property<bool> visited;
	Surface_mesh::Vertex_property<bool> border;

	float radius;           // radius of the ball
	double min_edge;        // min length of an edge 
	double max_edge;        // max length of an edge 
	double max_angle;       // max angle between 2 faces (cos(angle) actually)  

	std::list<FrontEdge> front;   
	std::list<FrontEdge> deads;
	std::vector<int> nb; // nb = number of fronts a vertex is into, this is used 
	// for the Visited and Border flags but adding topology may not be needed anymore
...
public:
	...
	BallPivoting(){}
	BallPivoting(SurfaceMeshModel * _mesh, double _radius = 0, double minr = 0.2, double angle = M_PI/2):
	radius(_radius), min_edge(minr), max_edge(1.8), max_angle(cos(angle)), last_seed(-1) 
	{
		mesh = _mesh;

		real_points	= mesh->vertex_property<Vector3>(VPOINT);
		points	= mesh->vertex_property<Vector3>("v:_point", Vector3(0));
		usedBit	= mesh->vertex_property<bool>("v:usedBit", false);
		...
		foreach(Vertex v, mesh->vertices()) {
			points[v] = real_points[v];// + (mesh->bbox().size().length() * 1e-3*Vector3(uniform(), uniform(), uniform()).normalized());
			tpoints.push_back( SpherePackSampling::toKDPoint(points[v]));
		}
		grid = new KDTree(tpoints);

		nb.clear();
		nb.resize(mesh->n_vertices(), 0);


		baricenter = Vector3(0);
		foreach(Vertex v, mesh->vertices()) baricenter += points[v];
		baricenter /= (double) mesh->n_vertices();

		if(radius == 0) // radius == 0 means that an auto guess should be attempted. 
			radius = sqrt((mesh->bbox().size().length() * mesh->bbox().size().length()) / mesh->n_vertices());
	...
	bool AddFace() 
	{
			...
			(touch.first == DEADS) && (touch.second == deads.end()))
		{
			...
			/         V
			----v0 - - - > v1--------- */
			assert(!border[Vertex(v2)]); //fatal error! a new point is already a border?
			nb[v2]++;                 
			border[Vertex(v2)] = true;
	...
	bool Seed(int &v0, int &v1, int &v2) {               
		...
		while(++last_seed < (int)mesh->n_vertices()) 
		{
			Vertex seed (last_seed);

			if(usedBit[seed]) continue; 
			usedBit[seed] = true;

			grid->ball_query( toKDPoint( points[seed] ), 2*radius, targets, dists );
			int n = targets.size();

			// SORT
			{
				// Sort by distance
				std::vector< std::pair<int, double> > sorted_dists;
				for(int u = 0; u < n; u++) sorted_dists.push_back(std::make_pair(targets[u],dists[u]));
				std::sort(sorted_dists.begin(), sorted_dists.end(), kd_sort_pred());

				// Reassign
				targets.resize(n, 0);
				for(int u = 0; u < n; u++) targets[u] = sorted_dists[u].first;
			}
			...
			for(int i = 0; i < n; i++) {
				vv0 = Vertex(targets[i]);
				Vector3 p0 = points[vv0];        

				for(int k = i+1; k < n; k++) {
					vv1 = Vertex(targets[k]);            
					...
					for(int j = k+1; j < n; j++) {
						...
						int t;
						for(t = 0; t < n; t++) {
							if((center - points[Vertex(targets[t])]).norm() * 1.001 <= radius)
								break;              
						}
						if(t < n) {
							continue;                         
						...
						for(t = 0; t < n; t++) {
							Vertex v = Vertex(targets[t]);
							if(visited[v] && (opposite - points[v]).norm() <= radius) 
								break;              
						}
						if(t < n) {
							continue;                         
	...
	int Place(FrontEdge &edge, ResultIterator &touch) {
		...
		Vector3 normal = cross((v1 - v0),(v2 - v0)).normalized();        
		Vector3 middle = (v0 + v1)/2.0;    
		Vector3 center;    
		...
		grid->ball_query( toKDPoint( middle), r + radius, targets, dists);
		int n = targets.size();
		...
		for(int i = 0; i < (int)targets.size(); i++) {
			...
			if(visited[v]) assert(usedBit[v]);

			if(usedBit[v] && !border[v]) continue;
		...
		Vector3 newnormal = cross((points[candidate] - v0),(v1 - v0)).normalized();
	...
	bool CheckEdge(int vv0, int vv1) {
		int tot = 0;
		...
		for(int f = 0; f < (int)fV.size(); f++)
		{
			for(int k = 0; k < 3; k++) 
			{
				if(vv0 == fV[f][k] && vv1 == fV[f][(k+1)%3])  // orientation non consistent
					return false;              
				else if(vv1 == fV[f][k] && vv0 == fV[f][(k+1)%3]) ++tot;
			}
	...
	void Mark(Vertex v) {
		...
		grid->ball_query(toKDPoint( points[v] ), min_edge, targets, dists);

		for(int t = 0; t < (int)targets.size(); t++) 
			usedBit[ Vertex(targets[t]) ] = true;
	...
	bool CheckFrontEdge(int v0, int v1) {
		...
		for(int f = 0; f < (int)fV.size(); f++)
		{
			for(int k = 0; k < 3; k++)
			{
				if(v1 == fV[f][k] && v0 == fV[f][(k+1)%3] )
					++tot;
				else if(v0 == fV[f][k] && v1 == fV[f][(k+1)%3]){ 
					// orientation non consistent
					return false;
				}
			}
			if(tot >= 2) { // non manifold
